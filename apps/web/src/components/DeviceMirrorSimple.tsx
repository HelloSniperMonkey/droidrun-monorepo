...
export const DeviceMirror = () => {
    ...
    // Touch/mouse state for gesture detection
    ...
    const connect = useCallback(() => {
        ...
        try {
            ...
            ws.onopen = () => {
                ...
                // Start ping interval for latency measurement and FPS counter
            ...
            ws.onmessage = (event) => {
                ...
                // Handle binary image data (MJPEG frames)
            ...
            ws.onerror = () => {
                ...
                setError("Failed to connect to mirror service. Is it running on port 8080?");
                setConnectionState('error');
            };
        } catch (e) {
    ...
    // Helper function to calculate normalized coordinates
    const getNormalizedCoords = useCallback((clientX: number, clientY: number) => {
        if (!imgRef.current || !deviceInfo) return null;
        
        const rect = imgRef.current.getBoundingClientRect();
        const imgAspect = deviceInfo.width / deviceInfo.height;
        ...
        return {
            x: Math.max(0, Math.min(1, posX / imgWidth)),
            y: Math.max(0, Math.min(1, posY / imgHeight))
        };
    }, [deviceInfo]);

    // Mouse down handler for desktop swipe support
    const handleMouseDown = useCallback((e: React.MouseEvent<HTMLImageElement>) => {
        if (!imgRef.current || connectionState !== 'connected' || !deviceInfo) return;
        e.preventDefault();
    ...
    }, [connectionState, deviceInfo, getNormalizedCoords]);

    // Mouse move handler
    const handleMouseMove = useCallback((e: React.MouseEvent<HTMLImageElement>) => {
        if (!isDraggingRef.current || !touchStartRef.current) return;
        e.preventDefault();
        // We don't do anything during drag - just wait for mouse up
    }, []);

    // Mouse up handler for desktop swipe support
    const handleMouseUp = useCallback((e: React.MouseEvent<HTMLImageElement>) => {
        if (!imgRef.current || !touchStartRef.current || !deviceInfo) return;
        e.preventDefault();
        ...
        if (distance > 0.03 && isDraggingRef.current) {
            // It's a swipe - use appropriate duration for smooth swipe
            const swipeDuration = Math.max(150, Math.min(duration, 400));
            sendCommand({
                type: 'swipe',
    ...
    }, [sendCommand, deviceInfo, getNormalizedCoords]);
    ...
    const handleTouchStart = useCallback((e: React.TouchEvent<HTMLImageElement>) => {
        if (!imgRef.current || !deviceInfo) return;
        e.preventDefault();
    ...
    }, [deviceInfo, getNormalizedCoords]);
                ...
                {connectionState === 'error' ? (
                    <div className="p-6 max-w-xs text-center">
                        ...
                        <p className="text-xs text-gray-600 mb-4">
                            Make sure the mirror service is running:<br />
                            <code className="text-pink-400">cd apps/mirror-service && npm start</code>
                        </p>
                        <Button
                            variant="outline"
                            size="sm"
